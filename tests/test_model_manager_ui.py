...
def test_model_manager_uses_plain_rows_and_english_task_names(manager):
    ...
    sam2 = _row_for(manager, "SAM2 Hiera Tiny")
    assert sam2.text(0) == "기본 제공"
    assert sam2.text(1) == "Segmentation"
    assert sam2.text(3) == "내장"
    assert not hasattr(manager, "sam2_download_button")
    assert not hasattr(manager, "sam2_apply_button")
