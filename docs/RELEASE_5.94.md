...
Windows SDK CI는 C++17 런타임과 동일한 Visual Studio 2022 러너를 명시하고 CMake 경로 인수를 PowerShell 문자열로 전달한다. GitHub의 `windows-latest` 이미지 변경으로 설치파일 검증이 잘못 실패하던 문제도 함께 수정했다.
